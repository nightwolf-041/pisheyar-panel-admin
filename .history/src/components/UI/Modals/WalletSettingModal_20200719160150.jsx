...
function WalletSettingModal(props) {
  ...
  const [initialCreditInputLoading, setInitialCreditInputLoading] = React.useState('');
  const [requestsPriceInputValue, setRequestsPriceInputValue] = React.useState('');
  const [requestsPriceInputLoading, setRequestsPriceInputLoading] = React.useState('');
  ...
  const changeInitialCredit = () => {
    let reg = /^\d+$/;
    if(initialCreditInputValue === '' || initialCreditInputValue === null || initialCreditInputValue === undefined){
      setErrorOnInitialCreditModal(true)
    }else if((initialCreditInputValue !== '' || initialCreditInputValue !== null || initialCreditInputValue !== undefined) && !reg.test(initialCreditInputValue)) {
      setErrorOnNaNInitialCredit(true)
    }else{
      setErrorOnInitialCreditModal(false)
      setErrorOnNaNInitialCredit(false)
      setInitialCreditInputLoading(true)
    }
  }

  const changeRequestsPrice = () => {
    let reg = /^\d+$/;
    if(requestsPriceInputValue === '' || requestsPriceInputValue === null || requestsPriceInputValue === undefined){
      setErrorOnRequestsPriceModal(true)
    }else if((requestsPriceInputValue !== '' || requestsPriceInputValue !== null || requestsPriceInputValue !== undefined) && !reg.test(requestsPriceInputValue)) {
      setErrorOnNaNRequestsPrice(true)
    }else{
      setErrorOnRequestsPriceModal(false)
      setErrorOnNaNRequestsPrice(false)
      setRequestsPriceInputLoading(true)
      axiosConfig.post('/')
    }
  }
