...
function CreateAdvertisementModal(props) {
  ...
  const [advertisementURL, serAdvertisementURL] = React.useState('');
  const [advertisementURLError, serAdvertisementURLError] = React.useState(false);
  const [advertisementAbstract, serAdvertisementAbstract] = React.useState('');
  const [advertisementAbstractError, serAdvertisementAbstractError] = React.useState(false);
  const [advertisementDescription, setAdvertisementDescription] = React.useState('');
  const [advertisementDescriptionError, setAdvertisementDescriptionError] = React.useState(false);
  const [checked, setChecked] = React.useState(true);
  const [advertisementCreateEmptyError, setAdvertisementCreateEmptyError] = React.useState(false);
  ...
  const categoriesSetDetailsHandler = () => {
    if(documentGuidForCover === null || advertisementURL === '' || advertisementURL === ' ' || advertisementAbstract === '' || advertisementAbstract === ' ' || advertisementAbstract === null || advertisementDescription === '' || advertisementDescription === ' ' || advertisementDescription === null) {
        setAdvertisementCreateEmptyError(true)
        toast('لطفا مقادیر را پر کنید', {type: toast.TYPE.WARNING});
    }else{
        console.log(documentGuidForCover);
        setAdvertisementCreateLoading(true)
        setAdvertisementCreateEmptyError(false)
        axiosConfig.post('/Advertisement/Create', {
            title: advertisementAbstract,
            description: advertisementDescription,
            isShow: checked,
            documentGuid: documentGuidForCover.replace(/['"]+/g, '')
        }, {
  ...
  const advertisementURLInputHandler = e => {
    serAdvertisementURL(e.target.value)
  }
        ...
        <Fade in={props.showInfoModal}>
          ...
          <PerfectScrollbar>
            ...
            <Close className={classes.closeIocn} onClick={props.hideInfoModal} />
                ...
                // error={true}
                variant="outlined"
                onChange={(e) => advertisementURLInputHandler(e)}
                />
