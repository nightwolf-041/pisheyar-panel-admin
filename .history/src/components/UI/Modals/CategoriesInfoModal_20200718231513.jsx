...
function CategoriesInfoModal(props) {
  ...
  const [infoBoxTitle, setInfoBoxTitle] = React.useState('');
  const [defaultInfoBoxTitle, setDefaultInfoBoxTitle] = React.useState('');

  const [infoBoxAbstract, setInfoBoxAbstract] = React.useState('');
  const [defaultInfoBoxAbstract, setDefaultInfoBoxAbstract] = React.useState(true);
  ...
  React.useEffect(() => {
    // doSomething()
    setDefaultInfoBoxTitle(true)
    setDefaultInfoBoxAbstract(true)
    console.log(props.showInfoModal);
      ...
      axiosConfig.get('/Category/' + guid, {
        ...
        if(res.data.state === 1) {
          ...
          if(data.abstract !== null){
            ...
            setDefaultInfoBoxAbstract(false)
          }else{
            setInfoBoxAbstract(null)
            setDefaultInfoBoxAbstract(false)
          }

          if(data.title !== null){
            setInfoBoxTitle(data.title)
            setDefaultInfoBoxTitle(false)
          }else{
  ...
  }, [counter, props.showInfoModal, props.infoData])
  ...
  const infoBoxTitleInputHandler = e => {
    setInfoBoxTitle(e.target.value)
  }
          ...
          <Close className={classes.closeIocn} onClick={props.hideInfoModal} />
            ...
            <Divider id="transition-modal-divider" className={classes.marginBottom}/>
              ...
              <FilePond ref={ref => pond3 = ref}
                ...
                imagePreviewMaxHeight={300}
                allowImageValidateSize={true}
                imageValidateSizeMinWidth={60}
                imageValidateSizeMaxWidth={300}
                imageValidateSizeMinHeight={60}
              ...
              {!defaultInfoBoxTitle ?
                ...
                value={infoBoxTitle}
                variant="outlined"
                onChange={(e) => infoBoxTitleInputHandler(e)}
                />
              : 
              <Autocomplete
                options={fakeAutoOptions}
                id="InfoBoxTitle-desable"
                disabled
                style={{ direction: 'rtl' }}
                className={[classes.inputs, "inputsDir", classes.titleMarginTop].join(' ')}
